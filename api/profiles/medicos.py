...
class MedicosProfileRouter:
    ...
    async def update_medico(self, cmd: UpdtMedicoCommand, db: AsyncSession = Depends(get_db), current_user: dict = Depends(jwt_service.get_current_user)):

        if not current_user["id_user"]:
            raise WrongJsonToken("El token enviado no posee las especificaciones técnicas correctas. ", 400)

        await self.__medico_handler.update_perfil_medico(current_user["id_user"], cmd, db)
