...
class PacientesProfileRouter:
    ...
    async def create_paciente(self, cmd: AddPacienteCmd, db: AsyncSession = Depends(get_db), current_user: Users = Depends(get_current_user)):
        handler = PacienteProfileHandler(db)
        await handler.create_paciente(cmd)

    async def update_paciente(self, cmd: UpdtPacienteCommand, db: AsyncSession = Depends(get_db), current_user: Users = Depends(get_current_user)):
    ...
    async def delete_paciente(self, cmd: DeletePacienteCmd, db: AsyncSession = Depends(get_db), current_user: Users = Depends(get_current_user)):
        handler = PacienteProfileHandler(db)
