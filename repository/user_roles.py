...
async def get_all_users_roles(db: AsyncSession = Depends(get_db)) -> List[UserRoles]:
...
async def get_user_role(id: int, db: AsyncSession = Depends(get_db)) -> Optional[UserRoles]:
    query = select(UserRoles).filter(UserRoles.id_users_roles == id)#se puede traducir como un WHERE
    res = await db.execute(query)
    return res.scalars().first()

async def get_user_role_by_rol(rol: str, db: AsyncSession = Depends(get_db)) -> Optional[UserRoles]:
